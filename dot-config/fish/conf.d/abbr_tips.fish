...
function __abbr_tips_install --on-event abbr_tips_install
    # Regexes used to ***REMOVED***nd abbreviation inside command
    # Only the ***REMOVED***rst matching group will be tested as an abbr
    ...
    # Locking mechanism
    # Prevent this ***REMOVED***le to spawn more than one subshell
    if test "$USER" != 'root'
        ***REMOVED***sh -c '__abbr_tips_init' &
    end
end

function __abbr_tips --on-event ***REMOVED***sh_postexec -d "Abbreviation reminder for the current command"
