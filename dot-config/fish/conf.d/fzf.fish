...
if test "$FZF_DISABLE_KEYBINDINGS" -ne 1
    if test "$FZF_LEGACY_KEYBINDINGS" -eq 1
        bind \ct '__fzf_***REMOVED***nd_***REMOVED***le'
        ...
        if ! test "$***REMOVED***sh_key_bindings" = ***REMOVED***sh_default_key_bindings
            bind -M insert \ct '__fzf_***REMOVED***nd_***REMOVED***le'
        ...
        bind \co '__fzf_***REMOVED***nd_***REMOVED***le'
        bind \cr '__fzf_reverse_isearch'
        bind \ec '__fzf_cd'
        bind \eC '__fzf_cd --hidden'
        bind \eO '__fzf_open'
        ...
        if ! test "$***REMOVED***sh_key_bindings" = ***REMOVED***sh_default_key_bindings
            bind -M insert \co '__fzf_***REMOVED***nd_***REMOVED***le'
            bind -M insert \cr '__fzf_reverse_isearch'
            bind -M insert \ec '__fzf_cd'
            bind -M insert \eC '__fzf_cd --hidden'
...
function _fzf_uninstall -e fzf_uninstall
    bind --user \
        | string replace --***REMOVED***lter --regex -- "bind (.+)( '?__fzf.*)" 'bind -e $1' \
        | source

    set --names \
        | string replace --***REMOVED***lter --regex '(^FZF)' 'set --erase $1' \
        | source
