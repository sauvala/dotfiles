function __fzf_open -d "Open ***REMOVED***les and directories."
    ...
    if set -q FZF_ENABLE_OPEN_PREVIEW
        set preview_cmd "--preview-window=right:wrap --preview='***REMOVED***sh -c \"__fzf_complete_preview {}\"'"
    end

    set -q FZF_OPEN_COMMAND
    or set -l FZF_OPEN_COMMAND "
    command ***REMOVED***nd -L \$dir -mindepth 1 \\( -path \$dir'*/\\.*' -o -fstype 'sysfs' -o -fstype 'devfs' -o -fstype 'devtmpfs' \\) -prune \
    -o -type f -print \
    -o -type d -print \
    ...
    if set -q _flag_editor
        ...
        if test -z "$open_cmd"
            echo "Couldn't ***REMOVED***nd appropriate open command to use. Do you have 'xdg-open' or 'open' installed?"; and return 1
        end
    end
