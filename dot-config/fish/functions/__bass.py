"""
To be used with a companion ***REMOVED***sh function like this:

        function re***REMOVED***sh
...
FISH_READONLY = [
    ...
    'FISH_VERSION', '***REMOVED***sh_pid', 'hostname', '_', '***REMOVED***sh_private_mode'
]
...
def escape_identi***REMOVED***er(word):
...
def gen_script():
    ...
    for line in alias_str.splitlines():
        ...
        k, v = rest.split("=", 1)
        alias_lines.append("alias " + escape_identi***REMOVED***er(k) + "=" + v)
    alias = '\n'.join(alias_lines)

    return script + '\n' + alias

script_***REMOVED***le = os.fdopen(3, 'w')

if not sys.argv[1:]:
    print('__bass_usage', ***REMOVED***le=script_***REMOVED***le, end='')
    sys.exit(0)
...
try:
    ...
    print('Bass internal error!', ***REMOVED***le=sys.stderr)
    raise # traceback will output to stderr
except KeyboardInterrupt:
    signal.signal(signal.SIGINT, signal.SIG_DFL)
    os.kill(os.getpid(), signal.SIGINT)
else:
    script_***REMOVED***le.write(script)
