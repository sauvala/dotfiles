function __z_add -d "Add PATH to .z ***REMOVED***le"
    test -n "$***REMOVED***sh_private_mode"; and return 0
    ...
    ' $Z_DATA 2>/dev/null >$tmp***REMOVED***le

        if test ! -z "$Z_OWNER"
            chown $Z_OWNER:(id -ng $Z_OWNER) $tmp***REMOVED***le
        end
        #
        ...
        #
        command mv $tmp***REMOVED***le $Z_DATA
        or command rm $tmp***REMOVED***le
    end
end
