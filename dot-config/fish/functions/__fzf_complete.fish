##
# Use fzf as ***REMOVED***sh completion widget.
#
#
# When FZF_COMPLETE variable is set, fzf is used as completion
# widget for the ***REMOVED***sh shell by binding the TAB key.
#
...
# modi***REMOVED***ed from https://github.com/junegunn/fzf/wiki/Examples-(***REMOVED***sh)#completion
function __fzf_complete -d 'fzf completion and print selection back to commandline'
    # As of 2.6, ***REMOVED***sh's "complete" function does not understand
    # subcommands. Instead, we use the same hack as __***REMOVED***sh_complete_subcommand and
    ...
    if test $compwc -eq 1
        ...
        | cut -f1 \
        | while read -l r
            # ***REMOVED***rst line is the user entered query
            if test -z "$query"
                set query $r
    ...
    set pre***REMOVED***x (string sub -s 1 -l 1 -- (commandline -t))
...
function __fzf_complete_opts_preview
    set -l ***REMOVED***le (status -f)
    echo --with-nth=1 --preview-window=right:wrap --preview="***REMOVED***sh\ '$***REMOVED***le'\ __fzf_complete_preview\ '{1}'\ '{2..}'"
end
...
function __fzf_complete_opts -d 'fzf options for ***REMOVED***sh tab completion'
    switch $FZF_COMPLETE
        case 0
            __fzf_complete_opts_0
        case 1
